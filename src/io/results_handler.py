...
class ResultSaver:
    ...
    def save_hydrographs(self, hydrograph_dict: dict):
        """
        Saves a dictionary of full hydrographs to the HDF5 file. This is
        typically called once at the end of a phase.
        Args:
            hydrograph_dict (dict): {element_id: (times_tensor, q_tensor)}
        """
        if self.h5file is None or not self.h5file.id:
             # If the file is closed (e.g., after finalize()), re-open in append mode
            try:
                self.h5file = h5py.File(self.filepath, 'a')
                print(f"Re-opened {self.filepath} to append hydrographs.")
            except Exception as e:
                print(f"Error re-opening HDF5 file to save hydrographs: {e}")
                return

        print("Saving full hydrographs to HDF5...")
        hydrograph_group = self.h5file.require_group("hydrographs")

        for element_id, (times, q_values) in hydrograph_dict.items():
            elem_group = hydrograph_group.require_group(str(element_id))
            
            times_cpu = times.cpu().numpy()
            q_cpu = q_values.cpu().numpy()
        ...
        self.h5file.flush()
        print(f"Hydrographs for {len(hydrograph_dict)} elements saved.")
        # We don't close the file here, finalize() will handle it.
...
class ResultSaver_v2:
    ...
    def finalize(self):
        """Flushes any remaining data, trims HDF5 datasets, and closes the file."""
        if self.h5file and self.h5file.id:
            if self._data_buffer:
                print("Finalizing ResultSaver: Flushing remaining data from buffer...")
                self._flush_buffer_to_hdf5()
